...
import { Box, Typography, TextField, IconButton, Avatar, Divider, Paper, Chip, Button, Stack } from '@mui/material';
import { Send, FiberManualRecord, Shield, ExitToApp, NavigateNext, Flag, Terminal, Security } from '@mui/icons-material';
...
const Chat = () => {
  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh', width: '100vw', bgcolor: '#0a0a0a', color: '#fff', overflow: 'hidden' }}>
      <Box sx={{ width: 300, bgcolor: '#111', borderRight: '1px solid #222', display: 'flex', flexDirection: 'column', p: 3 }}>
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ p: 2, bgcolor: '#111', borderBottom: '1px solid #222', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
           ...
           <Chip label="E2EE ACTIVE" size="small" sx={{ color: '#00ff88', borderColor: '#00ff88', fontSize: '0.6rem' }} variant="outlined" />
        </Box>

        <Box ref={scrollRef} sx={{ flexGrow: 1, overflowY: 'auto', p: 3, display: 'flex', flexDirection: 'column' }}>
          <Box sx={{ flexGrow: 1 }} />
          {messages.map((msg, i) => (
            <Box key={i} sx={{ alignSelf: msg.sender === 'me' ? 'flex-end' : (msg.sender === 'system' ? 'center' : 'flex-start'), mb: 2 }}>
        ...
        <Box sx={{ p: 3, bgcolor: '#111', borderTop: '1px solid #222' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: '#000', border: '1px solid #333', borderRadius: '12px', px: 2 }}>
            <TextField fullWidth variant="standard" placeholder="Execute message..." value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} InputProps={{ disableUnderline: true, sx: { color: '#fff', py: 1.5 } }} />
